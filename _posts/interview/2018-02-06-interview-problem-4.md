...
> * 开始请求 ===》 获取首字节 ===》 页面开始展示 ===》 首屏内容加载完成

> * 开始请求：`performance.timing.navigationStart`
> * 获取首字节：`performace.timing.responseStart`
> * 页面开始展示（白屏时间）= 页面开始展示的时间点 - 发起请求的时间点（在head末尾测试时间终点）
> * 首屏内容加载完成（首屏时间）：
...
> * nodejs是服务器端的javascript的运行环境，让我们在本地或者服务器上运行代码
> * Node是基于事件驱动和无阻塞的，所以非常适合处理并发请求，因此构建在Node上的代理服务器相比其他技术实现（如Ruby）的服务器表现要好得多
> * nodejs有需要应用场景：搭建服务器、应用开发、构建工具。。。
...
> * 就像ajax里一样，如果是get方法，所发送的数据直接是null
